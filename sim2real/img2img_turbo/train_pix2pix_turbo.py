...
# Original path in upstream: src/train_pix2pix_turbo.py
#
# Minimal edits: switched `pix2pix_turbo` / `my_utils` imports to package-relative imports
# so training runs from the vendored `sim2real.img2img_turbo` package (no `src/` on PYTHONPATH).
...
def main(args):
    ...
    global_step = 0
    # Run until max_train_steps (cycle the dataloader); do not stop at num_training_epochs * len(dl_train) only.
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.viz_freq == 1:
                        log_dict = {
                            "train/source": [
                                wandb.Image(x_src[idx].float().detach().cpu(), caption=f"idx={idx}") for idx in range(B)
                            ],
                            "train/target": [
                                wandb.Image(x_tgt[idx].float().detach().cpu(), caption=f"idx={idx}") for idx in range(B)
                            ],
                            "train/model_output": [
                                wandb.Image(x_tgt_pred[idx].float().detach().cpu(), caption=f"idx={idx}")
                                for idx in range(B)
                            ],
                        }
                        for k in log_dict:
                        ...
                        for step, batch_val in enumerate(dl_val):
                            ...
                            with torch.no_grad():
                                x_tgt_pred = accelerator.unwrap_model(net_pix2pix)(
                                    x_src, prompt_tokens=batch_val["input_ids"].cuda(), deterministic=True
                                )
...
if __name__ == "__main__":
    _args = parse_args_paired_training()
    main(_args)
